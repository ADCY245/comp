...
# ---------------- In-memory cart cache helpers ----------------
cart_cache = {}
CACHE_TIMEOUT_SECONDS = 30  # seconds


def _get_cached_cart(user_id: str):
    entry = cart_cache.get(user_id)
    if entry:
        products, timestamp = entry
        if (datetime.utcnow() - timestamp).total_seconds() < CACHE_TIMEOUT_SECONDS:
            app.logger.debug("[CACHE] Returning cached cart for user %s", user_id)
            return products
        cart_cache.pop(user_id, None)
    return None


def _set_cached_cart(user_id: str, products):
    cart_cache[user_id] = (products, datetime.utcnow())
    app.logger.debug("[CACHE] Cached cart for user %s", user_id)


def _clear_cart_cache(user_id: Optional[str] = None):
    if user_id is None:
        cart_cache.clear()
        app.logger.debug("[CACHE] Cleared entire cart cache")
    else:
        cart_cache.pop(user_id, None)
        app.logger.debug("[CACHE] Cleared cache for user %s", user_id)

# -------------------- Cart helper wrappers --------------------

def get_user_cart():
    """Return a dict with a products list for the current user, using cache to minimise DB queries."""
    """Return a dict with a products list for the current user.
    Utilises in-memory caching to minimise MongoDB round-trips."""
    ...
    try:
        ...
        if MONGO_AVAILABLE and USE_MONGO and mongo_db is not None:
            ...
            try:
                ...
                # Cache the fetched products for subsequent requests
                _set_cached_cart(current_user.id, products)
...
def save_user_cart(cart_dict):
    ...
    try:
        ...
        if MONGO_AVAILABLE and USE_MONGO and mongo_db is not None:
            ...
            _clear_cart_cache(current_user.id)
        else:

...
# ---------------------------------------------------------------------------
def generate_quotation_email(products, customer_info, user, notes, quote_id, today):
    ...
    try:
        ...
        # Calculate discounts
        total_discount = sum(
            (float(p.get('unit_price', p.get('base_price', 0)) or 0) * 
             float(p.get('quantity', 1) or 1) * 
             float(p.get('discount_percent', 0) or 0) / 100)
            for p in products
        )
        
        # Calculate GST (12% for mpack, 18% for others)
        total_gst = sum(
            (float(p.get('unit_price', p.get('base_price', 0)) or 0) * 
             float(p.get('quantity', 1) or 1) * 
             (0.12 if p.get('type') == 'mpack' else 0.18))
            for p in products
        )
        
        total_post_gst = subtotal - total_discount + total_gst
        ...
        context = {
            ...
            'total_discount': total_discount,
            'total_gst': total_gst,
            'total_post_gst': total_post_gst,
            'show_discount': any(float(p.get('discount_percent', 0) or 0) > 0 for p in products)
        }
            ...
            <div class="section">
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th>Quantity</th>
                            <th>Unit Price</th>
                            {% if show_discount %}<th>Discount</th>{% endif %}
                            <th>Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                {% if item.get('type') %}<small>Type: {{ item.type|title }}</small><br>{% endif %}
                                {% if item.get('size') %}<small>Size: {{ item.size }}</small><br>{% endif %}
                                {% if item.get('thickness') %}<small>Thickness: {{ item.thickness }}</small>{% endif %}
                            </td>
                            ...
                            <td>
                                {% if item.get('discount_percent', 0) > 0 %}
                                    {{ "%.1f"|format(item.discount_percent|float) }}%
                                {% else %}-
                                {% endif %}
                            </td>
                            {% endif %}
                            <td>₹{{ "%.2f"|format((item.get('unit_price', item.get('base_price', 0))|float) * (item.get('quantity', 1)|int)) }}</td>
                        </tr>
                        {% endfor %}
                    </tbody>
                </table>
                ...
                <div class="totals">
                    <table>
                        ...
                        {% endif %}
                        <tr>
                            <td><strong>GST ({{ '12%' if any(p.get('type') == 'mpack' for p in products) else '18%' }}):</strong></td>
                            <td>₹{{ "%.2f"|format(total_gst) }}</td>
                        </tr>
                        <tr style="font-weight: bold; font-size: 1.1em;">
                            <td><strong>Total:</strong></td>
                            <td>₹{{ "%.2f"|format(total_post_gst) }}</td>
                        </tr>
                    </table>
                </div>
        ...
        app.logger.error(f"Error generating email content: {str(e)}")
        raise

def send_quotation_email(recipients, subject, html_content):
    """Send quotation email using Flask-Mail or your email service"""
    try:
        # Import here to avoid circular imports
        ...
        def send_async_email(app, msg):
            with app.app_context():
                try:
                    mail.send(msg)
                    app.logger.info(f"Email sent to {', '.join(recipients)}")
                except Exception as e:
        ...
        app.logger.error(f"Error in send_quotation_email: {str(e)}")
        return False
...
def send_quotation():
    ...
    try:
        ...
        # 5. Start background task for email sending
        from threading import Thread
        ...
        app.logger.error(f"Error in send_quotation: {str(e)}", exc_info=True)
        return jsonify({
